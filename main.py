...
for chunk in chunk_iterator:
  for index, row in chunk.iterrows():
    for suffix in ['', '.1', '.2', '.3', '.4', '.5']:
      if not isActuallyNan(row['Name'+suffix]):
        count += 1
...
for key, value in reasons_by_frequency.items():
   if len(key) < 2 or key in ['info', 'information', 'inv', 'investigation', 'suspect', '[blank or whitespace]', 'suspicious', 'investigations', 'tbd', 'search', 'query', 'investigate', 'training', 'research', 'invst', 'investigaton', '---', 'suspicious activity', 'location', ]:
     bs_reason_count += value
   if value > 100:
    print(f'{key}: {value}')

print('------')
print(f'in state: {instate_count}, {100*instate_count/count}% of total')
print(f'with a case: {has_case_count}, {100*has_case_count/count}% of total')
print(f'with a short, generic, or blank reason: {bs_reason_count}, {100*bs_reason_count/count}% of total')
print(f'total: {count}')